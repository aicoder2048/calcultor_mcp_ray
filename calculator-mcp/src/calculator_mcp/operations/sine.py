...
class SineOperation(BaseOperation):
    ...
    def description(self) -> str:
        return "计算角度的正弦值，支持角度(degree)和弧度(radian)两种单位"
    
    @property
    def input_model(self) -> Type[BaseModel]:
        return SineInput
    
    def validate_input(self, input_data: SineInput) -> bool:
        if not math.isfinite(input_data.angle):
            return False
        return True
    
    async def execute(self, input_data: SineInput) -> OperationResult:
